          ...
          {section.items.map((item) => {
            ...
            const link = (
              <Link
                to={item.to}
                onClick={() => {
                  if (closeOnNavigate) setMobileNavOpen(false);
                }}
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-muted/40 text-foreground">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col bg-sidebar text-sidebar-foreground lg:flex">
        ...
        <nav className="flex-1 overflow-y-auto scrollbar-thin px-3 py-4">{renderNav()}</nav>
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 border-b border-border bg-background/80 backdrop-blur-xl">
          <div className="flex min-h-16 items-center gap-2 px-3 sm:gap-4 sm:px-4 md:px-6">
            <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
              ...
              <SheetContent
                side="left"
                className="w-[min(19rem,85vw)] bg-sidebar p-0 text-sidebar-foreground"
              >
                <SheetHeader className="border-b border-sidebar-border px-5 py-5 text-left">
                  <SheetTitle className="flex items-center gap-3 text-sidebar-foreground">
                    <span className="flex h-9 w-9 items-center justify-center rounded-xl gradient-primary">
                      <Activity className="h-4 w-4 text-primary-foreground" />
                    </span>
                    <span>
                      <span className="block text-sm font-bold tracking-tight">MIKAPEDIA</span>
                      <span className="block text-[10px] font-normal uppercase tracking-[0.18em] text-sidebar-foreground/60">
                        TOMS · v1.0
                      </span>
                    </span>
                  </SheetTitle>
                </SheetHeader>
                <nav className="h-[calc(100%-5.5rem)] overflow-y-auto px-3 py-4">
                  {renderNav(true)}
                </nav>
              </SheetContent>
            </Sheet>
